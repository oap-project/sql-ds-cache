...
inline const uint32_t* unpack32(const uint32_t* in, char* out) {
  uint32_t inl = *in;
  *out = (inl >> 0) & 1;
  out++;
  *out = (inl >> 1) & 1;
...
void unpack(const uint32_t* in, char* out, int batch_size) {
  int i = 0;
...
int32_t QplFilter(qpl::comparators comparator, bool inclusive, uint32_t value,
      const uint8_t *in, char* out, int32_t batch_size) {
  ...
  const auto scan_result = qpl::execute<qpl::auto_detect>(scan_operation,
      in, in + batch_size * sizeof(uint32_t),
      std::begin(bitmask),
      std::end(bitmask));

  scan_result.handle([&bitmask, &out](uint32_t scan_size) -> void
      {
        const auto *indices = reinterpret_cast<const uint32_t *>(bitmask.data());
        unpack(indices, out, scan_size);
        ...
        try {
          ...
          ARROW_LOG(ERROR) << "Failed to submit scan job: " << "Unknown Exception";
        }
      });
