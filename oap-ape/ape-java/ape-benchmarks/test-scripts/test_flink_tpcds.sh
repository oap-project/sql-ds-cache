...
QUERY_DIR="$TPCDS_TEST_DIR/tpcds/queries"

TARGET_DIR="$TPCDS_TEST_DIR/target"
...
$FLINK_DIR/bin/flink run -t yarn-session -Dyarn.application.id=$YARN_APP_ID \
    -p $PARALLELISM -c org.apache.flink.table.tpcds.TpcdsTestProgram "$TARGET_DIR/TpcdsTestProgram.jar" \
