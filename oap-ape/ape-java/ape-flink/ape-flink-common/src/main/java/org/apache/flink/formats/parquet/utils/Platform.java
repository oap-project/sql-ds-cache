...
/** To control native memory with unsafe API. */
public final class Platform {
    ...
    static {
        ...
        try {
            ...
            // hack below. It doesn't break, just means the user might run into the default JVM
            // limit
    ...
    public static ByteBuffer allocateDirectBuffer(int size) {
        try {
            if (CLEANER_CREATE_METHOD == null) {
                // Can't set a Cleaner (see comments on field), so need to allocate via normal Java
                // APIs
                try {
            ...
            // Otherwise, use internal JDK APIs to allocate a DirectByteBuffer while ignoring the
            // JVM's
            ...
            try {
                DBB_CLEANER_FIELD.set(
                        ...
                        CLEANER_CREATE_METHOD.invoke(
                                null, buffer, (Runnable) () -> freeMemory(memory)));
                ...
                if (_UNSAFE != null && majorVersion >= 9) {
                    ...
                    Field unalignedField =
                            bitsClass.getDeclaredField(
                                    majorVersion >= 11 ? "UNALIGNED" : "unaligned");
                    unalignedTemp =
                            _UNSAFE.getBoolean(
                    ...
                    unalignedTemp = Boolean.TRUE.equals(unalignedMethod.invoke(null));
                }
            } catch (Throwable t) {
                // We at least know x86 and x64 support unaligned access.
                //noinspection DynamicRegexReplaceableByCompiledPattern
                unalignedTemp = arch.matches("^(i[3-6]86|x86(_64)?|x64|amd64|aarch64)$");
            }
        }
        unaligned = unalignedTemp;
    }
}
