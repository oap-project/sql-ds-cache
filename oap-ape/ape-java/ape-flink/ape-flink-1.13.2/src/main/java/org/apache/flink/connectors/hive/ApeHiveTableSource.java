...
public class ApeHiveTableSource
    ...
    protected DataStream<RowData> getDataStream(StreamExecutionEnvironment execEnv) {
        ...
        ApeHiveSource.HiveSourceBuilder sourceBuilder =
                new ApeHiveSource.HiveSourceBuilder(
                        jobConf,
                        tablePath,
                        catalogTable,
                        allHivePartitions,
    ...
    private void setFilterPredicate(FilterPredicate parquetPredicate) {
        ...
        try {
            SerializationUtil.writeObjectToConfAsBase64(
                    JOB_CONF_KEY_PREDICATES, parquetPredicate, jobConf);
    ...
    private void setAggregateExpr(AggregateExprs aggregateExprs) {
        ...
        try {
            SerializationUtil.writeObjectToConfAsBase64(
                    JOB_CONF_KEY_AGGREGATES, aggregateExprs, jobConf);
        } catch (IOException ex) {
            throw new RuntimeException(ex);
        }
    }
    ...
    public boolean applyAggregations(
            List<String> outputNames, List<DataType> outputTypes, AggregateExprs aggregateExprs) {
    ...
    private boolean isAPENativeFilterApplicable() {
        ...
        // This may lead to worse efficiency when it moves backward tables with filters pushed down
        // in joins.
