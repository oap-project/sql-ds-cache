...
public class ApeYarnResourceManagerDriver extends AbstractResourceManagerDriver<YarnWorkerNode> {
    ...
    private ContainerLaunchContext createTaskExecutorLaunchContext(
        ...
        if (numaNodeList.size() > 0) {
            ...
            numaBindingNodeCounter.putIfAbsent(
                    host, new AtomicInteger(new Random().nextInt(numaCount)));
            AtomicInteger current = numaBindingNodeCounter.get(host);
            int numaIndex = current.incrementAndGet() % numaCount;
            taskManagerParameters
                    .taskManagerEnv()
                    .put(NUMA_BINDING_NODES_INDEX_CONF_KEY, String.valueOf(numaIndex));
        ...
        final ContainerLaunchContext taskExecutorLaunchContext =
                ApeUtils.createTaskExecutorContext(
                        flinkConfig,
                        yarnConfig,
                        configuration,
                        taskManagerParameters,
