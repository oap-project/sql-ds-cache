...
public class ExpressionToPredicateConverter implements ExpressionVisitor<FilterPredicate> {
    ...
    private FilterPredicate notEquals(Tuple2<Operators.Column, Comparable> columnPair) {
        if (columnPair.f0 instanceof Operators.IntColumn) {
            ...
            return FilterApi.notEq(
                    (Operators.BooleanColumn) columnPair.f0, (Boolean) columnPair.f1);
        } else if (columnPair.f0 instanceof Operators.BinaryColumn) {
    ...
    private Tuple2<Operators.Column, Comparable> extractColumnAndLiteral(
        ...
        if (column != null && value != null && isDecimalColumn(children)) {
        ...
        if (obj instanceof Date) {
            ...
            LocalDate localDate =
                    LocalDate.of(
                                    ...
                                    // For example, 1000-02-29 exists in Julian calendar because
                                    // 1000
                                    // is a leap year but it is not a leap year in Gregorian
                                    // calendar.
                                    1)
    ...
    private FilterPredicate like(
            Tuple2<Operators.Column, Comparable> columnPair, List<ResolvedExpression> children) {
    ...
    /**
     * Try to get a `startsWith` value from a `like` pattern. For example, "123%_" => "123".
     * "123%abc%" => null.
     *
     * @param likeValue String
     * @return String the captured value contains no wildcards, or null if the pattern does not just
     *     mean `startsWith`.
     */
    @Nullable
    ...
    /**
     * Try to get a `endsWith` value from a `like` pattern. For example, "%123" => "123". "%123_abc"
     * => null.
     *
     * @param likeValue String
     * @return String the captured value contains no wildcards, or null if the pattern does not just
    ...
    /**
     * Try to get a `contains` value from a `like` pattern. For example, "%123%%" => "123".
     * "%123%abc%" => null.
     *
     * @param likeValue String
